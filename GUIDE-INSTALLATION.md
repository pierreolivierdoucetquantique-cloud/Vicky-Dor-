...
## ⚠️ Persistance des données en production (Render)

**À lire avant tout déploiement en ligne — sans cette étape, toutes les données enregistrées par Vicky (disponibilités, créneaux bloqués, bannière promo, etc.) seront effacées au déploiement suivant.**

Sur Render (et tout hébergeur similaire), le disque du service web est **éphémère** : il est entièrement reconstruit à chaque redéploiement. Tout ce que le serveur écrit pendant son fonctionnement (la base de données SQLite, les images de bannière promo téléversées) disparaît si ce disque n'est pas rendu persistant.

**Configuration à faire une seule fois, sur Render :**

1. Ouvrir le service web sur le tableau de bord Render.
2. Aller dans l'onglet **Disks**.
3. Ajouter un disque persistant — 1 Go est largement suffisant pour ce projet.
4. Choisir un point de montage, par exemple : `/data`
5. Aller dans l'onglet **Environment** et ajouter la variable :
   - `DATA_DIR` = `/data` (exactement le même chemin que le point de montage choisi à l'étape 4)
6. Redéployer (Clear build cache & deploy).

Au premier démarrage après cette configuration, le serveur **migre automatiquement** les données déjà existantes (base de données et images) vers ce nouveau disque persistant — rien n'est perdu lors de la bascule. Tous les déploiements suivants préserveront alors les données indéfiniment.

En local (sur l'ordinateur du développeur), il n'est pas nécessaire de définir `DATA_DIR` : le serveur utilise automatiquement un dossier `data` à côté du code, comme avant.
