...
db.exec(`
  ...
  -- Messages soumis via le formulaire de contact public. Stockés en base pour
  -- que rien ne soit jamais perdu, même si l'envoi du courriel à Vicky échoue
  -- (clé Resend manquante, domaine non vérifié, problème réseau, etc.).
  CREATE TABLE IF NOT EXISTS contact_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    email TEXT NOT NULL,
    message TEXT NOT NULL,
    email_sent INTEGER NOT NULL DEFAULT 0, -- 1 si le courriel à Vicky a bien été envoyé
...
// ============================================================
// FORMULAIRE DE CONTACT — accessible sans compte, envoie un courriel à Vicky
// ============================================================
app.post('/api/contact', async (req, res) => {
  try {
    ...
    if (!email || !String(email).trim()) {
      return res.status(400).json({ error: 'L\'adresse courriel est requise.' });
    }
    ...
    // Le message est TOUJOURS enregistré en base, même si le courriel échoue :
    // ainsi Vicky peut le consulter depuis l'espace admin (onglet Messages)
    // sans jamais rien perdre, quelle que soit la configuration Resend.
    const result = db.prepare(`
      INSERT INTO contact_messages (name, email, message, email_sent) VALUES (?, ?, ?, 0)
    `).run(trimmedName, trimmedEmail, trimmedMessage);

    let emailSent = false;
    try {
      await sendContactFormEmail({ name: trimmedName, email: trimmedEmail, message: trimmedMessage });
      emailSent = true;
    } catch (mailErr) {
      console.error('⚠️  Erreur envoi courriel formulaire de contact (message tout de même enregistré) :', mailErr.message);
    }

    if (emailSent) {
      db.prepare('UPDATE contact_messages SET email_sent = 1 WHERE id = ?').run(result.lastInsertRowid);
    }

    res.json({ success: true });
  } catch (err) {
    console.error('Erreur traitement formulaire de contact :', err.message);
    res.status(500).json({ error: 'Erreur lors de l\'envoi du message. Veuillez réessayer ou écrire directement à Vicky_dore@hotmail.com.' });
  }
});

// Liste des messages de contact pour l'admin (les plus récents et non lus en premier).
...
app.post('/api/admin/contact-messages/:id/read', requireAdmin, (req, res) => {
  const { isRead } = req.body;
  db.prepare('UPDATE contact_messages SET is_read = ? WHERE id = ?').run(isRead ? 1 : 0, req.params.id);
  res.json({ success: true });
});
...
async function sendConfirmationEmails({ clientName, clientEmail, service, bookingDateLabel, bookingTime, paymentMethod, status, notifyAdmin = true }) {
  ...
  await resend.emails.send({ from: fromAddress, to: clientEmail, subject: `Votre réservation — ${statusLabel} — Vicky Doré`, html: clientHtml });
...
// ============================================================
// Courriel : message envoyé via le formulaire de contact du site
// ============================================================
async function sendContactFormEmail({ name, email, message }) {
  ...
  const html = `
    <div style="font-family:Georgia,serif;color:#352B28;max-width:480px;margin:0 auto;">
      <h3 style="color:#A87C2E;">Nouveau message — formulaire de contact ✦</h3>
      <p><strong>${escapeHtml(name)}</strong></p>
      <p><a href="mailto:${escapeHtml(email)}" style="color:#A87C2E;">${escapeHtml(email)}</a></p>
      <p style="white-space:pre-wrap;border-left:3px solid #E8DCC8;padding-left:12px;margin-top:16px;">${escapeHtml(message)}</p>
      <p style="margin-top:20px;font-size:13px;color:#8A7A74;">Vous pouvez répondre directement à ce courriel : la réponse sera envoyée à ${escapeHtml(email)}.</p>
    </div>
  `;
