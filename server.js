...
// IMPORTANT (Render) : le système de fichiers du service est éphémère et est
// réinitialisé à chaque redéploiement. DATA_DIR doit pointer vers le disque
// persistant monté sur Render (ex : /data) pour que la base de données et les
// images téléversées survivent aux redéploiements. En local (sans DATA_DIR
// défini), on retombe sur un dossier "data" à la racine du projet.
const DATA_DIR = process.env.DATA_DIR || path.join(__dirname, 'data');
fs.mkdirSync(DATA_DIR, { recursive: true });

const db = new Database(path.join(DATA_DIR, 'vickydore.db'));
...
const PRIVACY_POLICY_VERSION = '2026-09-04';
...
// Les images sont stockées sur le disque persistant (DATA_DIR), pas dans
// public/, pour survivre aux redéploiements Render. On les sert via une
// route statique dédiée sous /uploads.
