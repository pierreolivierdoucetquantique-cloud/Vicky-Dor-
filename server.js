...
async function sendConfirmationEmails({ clientName, clientEmail, service, bookingDateLabel, bookingTime, paymentMethod, status, notifyAdmin = true }) {
  ...
  console.log('📧 [DEBUG] Horodatage (serveur)  :', new Date().toISOString());
  ...
  try {
    const rawRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: fromAddress, to: clientEmail, subject: `Votre réservation — ${statusLabel} — Vicky Doré`, html: clientHtml }),
    });
    const rawText = await rawRes.text();
    console.log('📧 [DEBUG-RAW] Statut HTTP (client) :', rawRes.status, rawRes.statusText);
    console.log('📧 [DEBUG-RAW] Corps brut (client)  :', rawText);
  } catch (sendErr) {
    console.error('❌ [DEBUG-RAW] Erreur réseau brute (client) :', sendErr && sendErr.message, sendErr && sendErr.stack);
  }
  ...
  if (notifyAdmin) {
    ...
    try {
      const rawRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({ from: fromAddress, to: adminAddress, subject: `Nouvelle réservation : ${service.name}`, html: adminHtml }),
      });
      const rawText = await rawRes.text();
      console.log('📧 [DEBUG-RAW] Statut HTTP (admin) :', rawRes.status, rawRes.statusText);
      console.log('📧 [DEBUG-RAW] Corps brut (admin)  :', rawText);
    } catch (sendErr) {
      console.error('❌ [DEBUG-RAW] Erreur réseau brute (admin) :', sendErr && sendErr.message, sendErr && sendErr.stack);
    }
  }
}
